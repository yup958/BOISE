...
score_cpd <- function(P, sub_train, sub_inform, nT,  m0, a, b, row_sample_size, interm_size){
  ...
  if(is.null(sub_inform)){
    ...
    for (i in 1:l) {
      ...
      log_post_probs = matrix(0, 1, row_sample_size)
      post_thetas = matrix(0, row_sample_size, n)
      for (k in 1:row_sample_size){
      ...
      log_post_prob = -log(row_sample_size) + c + log(sum(exp(log_post_probs))) ## log(mean of post_probs)
      # E(theta | x0, xA)
      post_probs = exp(log_post_probs) / (sum(exp(log_post_probs)))
      tmp_score = post_probs %*% post_thetas
      Scores[[YA[i]]] = list(lg_wt = log_post_prob, 
                             sc = sort(tmp_score, decreasing = T)[1:min(nT, ncol(sub_train))])
    }
  }
  return(Scores)
